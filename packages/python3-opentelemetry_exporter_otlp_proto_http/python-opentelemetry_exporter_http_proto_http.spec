...
%global pypi_name opentelemetry-exporter-otlp-proto-http
...
BuildRequires:  (python3dist(requests) >= 2.7 with python3dist(requests) < 3)
BuildRequires:  python3dist(setuptools)

%description
OpenTelemetry Collector Protobuf over HTTP Exporter |pypi|.. |pyp This library
allows to export data to the OpenTelemetry Collector using the OpenTelemetry
Protocol using Protobuf over HTTP.Installation :: pip install opentelemetry-
exporter-otlp-proto-http References -* OpenTelemetry Collector Exporter < *
...
Requires:       (python3dist(requests) >= 2.7 with python3dist(requests) < 3)
...
%{python3_sitelib}/opentelemetry_exporter_otlp_proto_http-%{pypi_version}-py%{python3_version}.egg-info
